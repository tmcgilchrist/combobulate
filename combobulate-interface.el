...
(defun combobulate-primary-language (&optional quiet)
  "Get the primary language for the current buffer.

Returns a normalized symbol with hyphens (e.g., `ocaml-interface')
regardless of whether the tree-sitter parser was created with
underscores or hyphens.  This is the single point of
normalization for tree-sitter language names in Combobulate."
  (declare-function combobulate-normalize-language-name "combobulate-setup")
  (or
   (when-let ((ts-lang (treesit-language-at (point))))
     (combobulate-normalize-language-name ts-lang))
   (car (combobulate-get-registered-language major-mode))
   (when-let ((first-parser (car (combobulate-parser-list))))
     (combobulate-normalize-language-name (combobulate-parser-language first-parser)))
   (unless quiet
